...
const app = express();
let prefix = Config.HANDLERS === "null" ? '' : Config.HANDLERS[0];
...
//=============================[Implementing Promote and Demote Messages.]===================================================
                            ...
                            let buttonMessage = { image: {url : ppUrl }, caption: `[ PROMOTE - DETECTED ]\n\nName : @${num.split("@")[0]}\nStatus : Member -> Admin\nGroup : ${metadata.subject}`, mentions: [num],    }
                            return Void.sendMessage(anu.id, buttonMessage)
                        } else if (anu.action == 'demote') {                            
                            try {  ppUrl = await Void.profilePictureUrl(num, 'image'); }
                            catch { ppUrl = 'https://telegra.ph/file/93f1e7e8a1d7c4486df9e.jpg'; }
                            let buttonMessage = { image: { url : ppUrl },  caption: `[ DEMOTE - DETECTED ]\n\nName : @${num.split("@")[0]}\nStatus : Admin -> Member`, mentions: [num],      }
                            return Void.sendMessage(anu.id, buttonMessage)
                        }
                    }
                }
