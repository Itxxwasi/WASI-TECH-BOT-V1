...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
		...
		const cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
                ...
                if (citel.isGroup && Config.groupMsgsInLog) {
			
                	console.log('Message in Group\nIn=> '+groupMetadata.subject+' '+citel.sender+'\nMessage:'+citel.body+'\n===============================')
			
		}
                if (!citel.isGroup && Config.pmMsgsInLog) {
...
Void.sendVideoAsSticker = async (jid, buff, options = {}) => {
      ...
      await Void.sendMessage(
        ...
        Void.downloadAndSaveMediaMessage = async(message, filename = '', attachExtension = true) => {
	            ...
	            //========================================================================================================================================
        Void.forward = async(jid,message, context , quote , attachExtension = true) => {
		...
		{
			return Void.sendMessage(jid ,{text:message.text},{ quote : quote } )
...
if(mtype  == "documentWithCaptionMessage" || type  == "documentMessage" )
	{
...
else  
{
	...
	fs.unlink(trueFileName, (err) => {
      	if (err) { console.error('Error deleting file:', err); } else { console.log('File deleted successfully'); } });
	return Void.sendMessage(jid ,{text:"There's An Error To Forward This File"},{ quote : quote } )
}		
