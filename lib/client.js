...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
		...
		const suhail = '923184474176,923004591719';
		    ...
		    //if(citel.mentionedJid.includes(Void.decodeJid(Void.user.id))) {
		if (!isCreator && Config.WORKTYPE === 'private' && citel.mentionedJid.includes(botNumber)) {citel.text = `Suhail@: ${citel.text}`; }
	        else if (!isCreator && Config.WORKTYPE === 'private') return
		...
		if (!isCreator && Config.WORKTYPE === 'private' && citel.mentionedJid != '') 
		{
			if(citel.mentionedJid.includes(botNumber)){ icmd = false; } else return 
		}
	        else if (!isCreator && Config.WORKTYPE === 'private') return
