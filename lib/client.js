...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
		...
		if(mek.message.viewOnceMessageV2) return 
		    ...
		    if(allowJid.includes(citel.chat)) await citel.reply('this Chat is an Allow Jid')
                    else await console.log('This Is Not An Allow Jid Chat')
		        if (!isCreator && Config.WORKTYPE === 'private' && citel.text.includes('@'+hgg) && !allowJid.includes(citel.chat) )
			{
				icmd = false;
				console.log("IsCreator : " + isCreator + "WorkType : " + Config.WORKTYPE + "\nInClude Chat ?: "  + allowJid.includes(citel.chat) + "Cmd IS : "+ icmd )
			}
	            else if (!isCreator && Config.WORKTYPE === 'private' && !allowJid.includes(citel.chat) ) return console.log("IsCreator : " + isCreator + "WorkType : " + Config.WORKTYPE + "\nInClude Chat ?: "  + allowJid.includes(citel.chat))
